<script lang="ts">
    ...
    async function getProfilePictureUrl(userId: string): Promise<string | null> {
        try {
            ...
            // Assuming profile pictures are stored as: profilePictures/{userId}.jpg
            // You might need to adjust the path and extension based on your setup
            const profilePicRef = ref(storage, `profilePictures/${userId}.jpg`);
            const url = await getDownloadURL(profilePicRef);
            return url;
        } catch (error) {
            // If no profile picture exists, try other common extensions
            try {
    ...
    async function fetchLeaderboard() {
        try {
            ...
            // Query users ordered by totalCrosses in descending order
    ...
    {:else}
                ...
                <div class="flex items-center justify-between p-3 sm:p-4 bg-gray-50 rounded-lg border-l-4 {index < 3 ? 'border-blue-600' : 'border-gray-300'}">
                    ...
                    <div class="text-right flex-shrink-0 ml-8">
                        ...
                        <div class="text-xs sm:text-sm font-kalmansk text-black">
                            {user.totalCrosses === 1 ? 'kryss' : 'kryss'}
                        </div>
                    </div>
                </div>
            {/each}
        ...
        <div class="mt-4 sm:mt-6 pt-4 border-t border-gray-200">
            <div class="flex justify-center sm:justify-between text-xs sm:text-sm font-kalmansk text-black">
                <span class="text-center sm:text-left">Total mengde kryss: {leaderboard.reduce((sum, user) => sum + user.totalCrosses, 0)}</span>
            </div>
        </div>
    {/if}
</div>